...
function endpoints (endpoint = '') {
  ...
  } else { // else prefix it with endpointPrefix (often '/api/') so it calls our server api
...
endpoints.prototype.create = function (content) {
  return to(feathers.service(this.url).create(_.clone(content)))
}
...
endpoints.prototype.upsert = function (item) {
  ...
  if (rp._id) {
    ...
    return to(feathers.service(this.url).create(rp))
  }
}
