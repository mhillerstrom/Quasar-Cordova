...
const checkValid = async function checkValid (data, schema, field) {
  const foundSchema = typeof schema === 'string'
    ? _.get(schemas, schema) : schema

  console.log(`Data '${JSON.stringify(data)}', Schema Entered '${schema}', Found '${foundSchema}'`)
  if (!foundSchema) {
    return Promise.reject({ errors: ['Schema doesn\'t exist'] })
  }

  const [err, validData] = await to(
    foundSchema.validate(data, { abortEarly: false })
  )
  console.log(`After foundSchema.validate '${JSON.stringify(err)}', '${JSON.stringify(validData)}'...`)
